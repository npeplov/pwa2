...
const handlePush = (event: PushEvent) => {
  console.log("handlePush", event.data?.text())
...
self.addEventListener("push", (event) => {
  ...
  console.log("recieved  PUSH", JSON.stringify(event.data));
  // self.clients.matchAll().then(function (c) {
  // if (c.length === 0) {
  // event.waitUntil(new Promise((res) => res(handlePush(event))));
  // @ts-ignore
